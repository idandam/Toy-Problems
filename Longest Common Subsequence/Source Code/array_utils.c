...
//if the matrix is not random then nothing will be printed to the screen
//and the initial value at any entry will be zero
...
void addListNode(linkedlist_t** list, void* newData, size_t dataSize){
    ...
    for(int i = 0; i < dataSize; i++)
        *(char*)(newListNode->data + i) = *(char*)(newData + i);
...
void printLinkedList(linkedlist_t* list, void(*fptr)(void* data)){
    ...
    while(curr != NULL){
        //start execute instructions from the given address of the function (with the given parameter)
        (*fptr)(curr->data);
        curr=curr->next;
    }
    printf("\n");
